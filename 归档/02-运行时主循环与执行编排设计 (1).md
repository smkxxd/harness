...
<!-- 这是一个文本绘图，源码为：flowchart TD
    accTitle: Pico Runtime Main Loop
    accDescr: 这张图描述 Pico 如何把一条用户请求推进成一次独立 run，包括状态初始化、模型调用、工具执行和工件落盘。

    user["user request"]
    ask["Pico.ask()"]
    run["create TaskState + run dir"]
    prompt["build prompt + metadata"]
    model["model_client.complete()"]
    parse["parse raw output"]
    branch{"tool / final / retry"}
    tool["run_tool()"]
    state["update history / task_state / trace / memory"]
    stop["finish or stop"]
    report["write report.json"]

    user --> ask --> run --> prompt --> model --> parse --> branch
    branch -->|tool| tool --> state --> prompt
    branch -->|retry| state --> prompt
    branch -->|final| stop --> report
    prompt --> stop -->
