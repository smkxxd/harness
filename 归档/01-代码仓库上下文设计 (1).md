...
<!-- 这是一个文本绘图，源码为：flowchart TD
    accTitle: Pico Workspace Context Flow
    accDescr: 这张图描述 Pico 怎么在模型真正开始工作前，先收集一份仓库基线，再把它送进 prompt 前缀。

    cli["CLI build_agent()"]
    build["WorkspaceContext.build(cwd)"]
    gitinfo["Git facts + project docs"]
    text["WorkspaceContext.text()"]
    prefix["runtime.build_prefix()"]
    prompt["prompt sent to model"]
    refresh["refresh_prefix() before each round"]

    cli --> build --> gitinfo --> text --> prefix --> prompt
    prompt --> refresh --> build -->
![](https://cdn.nlark.com/yuque/__mermaid_v3/0350d1a219199e5e8143a1b4f4681791.svg)
