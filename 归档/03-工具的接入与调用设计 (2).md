...
<!-- 这是一个文本绘图，源码为：flowchart TD
    accTitle: Pico Tool Execution Chain
    accDescr: 这张图描述 Pico 里一条工具调用从模型输出到真正执行前，会经过哪些关口。

    model["model output"]
    registry["tool registry"]
    parse["structured tool call"]
    guard["runtime guards"]
    execute["tool runner"]
    result["history / trace / memory"]

    model --> parse --> registry --> guard --> execute --> result -->
![](https://cdn.nlark.com/yuque/__mermaid_v3/67739cbd21af9772c835ca55ebd12046.svg)
