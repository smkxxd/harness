...
from .repo_index import MAX_SEARCH_RESULTS, RepoIndex
...
BASE_TOOL_SPECS = {
    ...
    "search_repo": {
        "schema": {"query": "str", "path": "str='.'", "language": "str=''", "top_k": "int=8"},
        "risky": False,
        "description": "Search the code-aware repository index for relevant files and symbols.",
    },
    "get_symbol": {
        "schema": {"symbol": "str", "path": "str='.'", "context_lines": "int=4"},
        "risky": False,
        "description": "Read indexed definitions and nearby source lines for an exact symbol name.",
    },
    "run_shell": {
...
TOOL_EXAMPLES = {
    ...
    "search_repo": '<tool>{"name":"search_repo","args":{"query":"authentication token validation","top_k":6}}</tool>',
    "get_symbol": '<tool>{"name":"get_symbol","args":{"symbol":"TokenValidator","context_lines":6}}</tool>',
...
def validate_tool(context, name, args):
    ...
    if name == "search_repo":
        ...
        if top_k < 1 or top_k > MAX_SEARCH_RESULTS:
            raise ValueError(f"top_k must be in [1, {MAX_SEARCH_RESULTS}]")
        return
    ...
    if name == "get_symbol":
        ...
        if context_lines < 0 or context_lines > 20:
            raise ValueError("context_lines must be in [0, 20]")
        return
...
def _repo_index(context):
    ...
    return index if index is not None else RepoIndex(context.root)


def tool_search_repo(context, args):
    query = str(args.get("query", "")).strip()
    if not query:
        raise ValueError("query must not be empty")
    requested_path = context.path(args.get("path", "."))
    if not requested_path.is_dir():
        raise ValueError("path is not a directory")
    relative_path = requested_path.relative_to(context.root).as_posix() or "."
    results = _repo_index(context).search(
        query=query,
    ...
    lines = [f"repo_index_results: {len(results)}"]
    for rank, result in enumerate(results, start=1):
        symbols = ", ".join(
            f"{item['kind']} {item['name']}@{item['line']}"
            for item in result["symbols"]
        ) or "-"
        reasons = ", ".join(result["reasons"]) or "content"
        lines.extend(
            [
                f"{rank}. {result['path']}:{result['line']} "
                f"[{result['language']}, score={result['score']}]",
                f"   symbols: {symbols}",
                f"   matched_by: {reasons}",
                f"   snippet: {result['snippet'] or '(empty)'}",
            ]
        )
    return "\n".join(lines)


def tool_get_symbol(context, args):
    symbol = str(args.get("symbol", "")).strip()
    if not symbol:
        raise ValueError("symbol must not be empty")
    requested_path = context.path(args.get("path", "."))
    if not requested_path.is_dir():
        raise ValueError("path is not a directory")
    relative_path = requested_path.relative_to(context.root).as_posix() or "."
    results = _repo_index(context).symbol_context(
        symbol_name=symbol,
        path_filter=relative_path,
        context_lines=int(args.get("context_lines", 4)),
    )
