...
这个模块负责把“用户怎么启动 ForgeTrace”翻译成 runtime 能理解的对象：
...
from .runtime import ForgeTrace, SessionStore
...
WELCOME_NAME = "ForgeTrace"
WELCOME_SUBTITLE = "repository-aware coding agent"
...
def build_agent(args):
    """根据 CLI 参数装配出一个可运行的 ForgeTrace 实例。
    ...
    - 输出：一个新的 `ForgeTrace`，或一个从旧 session 恢复出来的 `ForgeTrace`
    ...
    if session_id:
        return ForgeTrace.from_session(
            model_client=model,
            workspace=workspace,
            session_store=store,
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        ...
        description="Repository-aware coding agent for DeepSeek, OpenAI-compatible, Anthropic-compatible, or Ollama models.",
    )
