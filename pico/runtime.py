...
ForgeTrace 是包在模型外面的控制循环：负责组 prompt、解析模型输出、
...
__all__ = ["ForgeTrace", "Pico", "SessionStore"]
...
class ForgeTrace:
    def __init__(
        ...
        # RepoIndex is shared by all repository-search tools in this runtime.
        # Its refresh operation only reparses files whose size/mtime changed.
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = ForgeTrace.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", ForgeTrace.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", ForgeTrace.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", ForgeTrace.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", ForgeTrace.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = ForgeTrace.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", ForgeTrace.retry_notice()
        if "<final>" in raw:
            final = ForgeTrace.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", ForgeTrace.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", ForgeTrace.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = ForgeTrace.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = ForgeTrace.extract_raw(body, key)
...
# Backward-compatible import for existing Pico sessions, tests and integrations.
# New code should use ForgeTrace.
Pico = ForgeTrace
