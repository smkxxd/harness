...
import pico
import forgetrace
from pico import Pico, ForgeTrace, SessionStore, WorkspaceContext, build_agent, build_arg_parser, build_welcome, main


def test_public_api_exports_current_names_only():
    assert Pico is not None
    assert ForgeTrace is Pico
    assert forgetrace.ForgeTrace is ForgeTrace
...
def test_packaging_discovers_forgetrace_and_compatibility_subpackages():
    pyproject_text = Path("pyproject.toml").read_text(encoding="utf-8")

    assert "[tool.setuptools.packages.find]" in pyproject_text
    assert 'include = ["pico*", "forgetrace*"]' in pyproject_text
